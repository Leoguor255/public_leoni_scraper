...
def save_failed_url_to_txt(url: str, source: str, reason: str, 
    ...
    """
    Append failed URL to text file in unified format for both scrapers.
    
    Args:
        url (str): Failed URL
        source (str): Source portal (PlanetBids/OpenGov)
        reason (str): Reason for failure
        city_name (str): City name (optional)
        project_title (str): Project title (optional)
        filename (str): Output filename (default: failed_urls.txt)
    """
    ...
    title_part = f" | {project_title[:50]}..." if project_title and len(project_title) > 50 else f" | {project_title}" if project_title else ""
    
    entry = f"{timestamp} | {source}{city_part}{title_part} | {url} | {reason}\n"
...
def save_failed_pages_batch(failed_pages: List[Dict], source: str, filename: str = "failed_urls.txt"):
    """
    Save multiple failed pages to text file in batch.
    
    Args:
        failed_pages (List[Dict]): List of failed page dictionaries
        source (str): Source portal (PlanetBids/OpenGov)
        filename (str): Output filename (default: failed_urls.txt)
    """
    if not failed_pages:
    ...
    print(f"💾 Saving {len(failed_pages)} failed URLs to {filename}...")
    
    for page in failed_pages:
        url = page.get('detail_url', page.get('url', ''))
        reason = page.get('reason', 'Unknown error')
        city_name = page.get('city_name', '')
        project_title = page.get('project_title', page.get('Project Title', ''))
        
        save_failed_url_to_txt(url, source, reason, city_name, project_title, filename)
...
def clear_failed_urls_file(filename: str = "failed_urls.txt"):
    ...
    header = f"""# Failed URLs Log - Generated on {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
# Format: TIMESTAMP | SOURCE | CITY | PROJECT_TITLE | URL | REASON
# This file is automatically populated during scraping runs
