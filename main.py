...
FILTER_DAYS = 7  # Only scrape bids from last X days
...
from san_fernando_scraper import scrape_all as san_fernando_scrape_all
...
def main() -> None:
    ...
    scraping_summary = {
        ...
        'san_fernando': {'success': False, 'records': 0, 'errors': []},
    ...
    try:
        ...
        if planet_stats and 'failed_pages' in planet_stats:
            failed_pages = planet_stats['failed_pages']
            if failed_pages:
                log_status('PlanetBids', 'Failed URLs', f'{len(failed_pages)} failed URLs collected', level='warning')
                all_failed_urls.extend([{**page, 'source': 'PlanetBids'} for page in failed_pages])
    ...
    try:
        ...
        if opengov_stats and 'failed_pages' in opengov_stats:
            failed_pages = opengov_stats['failed_pages']
            if failed_pages:
                log_status('OpenGov', 'Failed URLs', f'{len(failed_pages)} failed URLs collected', level='warning')
                all_failed_urls.extend([{**page, 'source': 'OpenGov'} for page in failed_pages])
    except Exception as e:
    ...
    try:
        ...
        if artesia_stats and 'failed_pages' in artesia_stats:
            failed_pages = artesia_stats['failed_pages']
            if failed_pages:
                log_status('Artesia', 'Failed URLs', f'{len(failed_pages)} failed URLs collected', level='warning')
                all_failed_urls.extend([{**page, 'source': 'Artesia'} for page in failed_pages])
    except Exception as e:
    ...
    try:
        ...
        if bell_gardens_stats and 'failed_pages' in bell_gardens_stats:
            failed_pages = bell_gardens_stats['failed_pages']
            if failed_pages:
                log_status('Bell Gardens', 'Failed URLs', f'{len(failed_pages)} failed URLs collected', level='warning')
                all_failed_urls.extend([{**page, 'source': 'Bell Gardens'} for page in failed_pages])
    except Exception as e:
    ...
    try:
        ...
        if calabasas_stats and 'failed_pages' in calabasas_stats:
            failed_pages = calabasas_stats['failed_pages']
            if failed_pages:
                log_status('Calabasas', 'Failed URLs', f'{len(failed_pages)} failed URLs collected', level='warning')
                all_failed_urls.extend([{**page, 'source': 'Calabasas'} for page in failed_pages])
    except Exception as e:
    ...
    try:
        ...
        if bidnet_stats and 'failed_pages' in bidnet_stats:
            failed_pages = bidnet_stats['failed_pages']
            if failed_pages:
                log_status('BidNet', 'Failed URLs', f'{len(failed_pages)} failed URLs collected', level='warning')
                all_failed_urls.extend([{**page, 'source': 'BidNet Direct'} for page in failed_pages])
    except Exception as e:
    ...
    try:
        ...
        if inglewood_stats and 'failed_pages' in inglewood_stats:
            failed_pages = inglewood_stats['failed_pages']
            if failed_pages:
                log_status('Inglewood', 'Failed URLs', f'{len(failed_pages)} failed URLs collected', level='warning')
                all_failed_urls.extend([{**page, 'source': 'Inglewood'} for page in failed_pages])
    except Exception as e:
    ...
    try:
        ...
        if san_gabriel_stats and 'failed_pages' in san_gabriel_stats:
            failed_pages = san_gabriel_stats['failed_pages']
            if failed_pages:
                log_status('San Gabriel', 'Failed URLs', f'{len(failed_pages)} failed URLs collected', level='warning')
                all_failed_urls.extend([{**page, 'source': 'San Gabriel'} for page in failed_pages])
    except Exception as e:
    ...
    # Run San Fernando scraper
    log_status('San Fernando', 'Start', 'Starting San Fernando scraper...')
    try:
        san_fernando_df, san_fernando_stats = san_fernando_scrape_all(date_filter=BID_FILTER_DATE)
        if not san_fernando_df.empty:
            log_status('San Fernando', 'Scrape', f'Raw data: {len(san_fernando_df)} records')
            san_fernando_airtable = san_fernando_df.copy()  # Already in Airtable format
            if not san_fernando_airtable.empty:
                all_airtable_data.append(san_fernando_airtable)
                scraping_summary['san_fernando']['success'] = True
                scraping_summary['san_fernando']['records'] = len(san_fernando_airtable)
                log_status('San Fernando', 'Airtable Mapping', f'Mapped for Airtable: {len(san_fernando_airtable)} records')
                save_airtable_format_csv(san_fernando_airtable.to_dict('records'), "san_fernando/san_fernando_bids.csv", "San Fernando")
            else:
                log_status('San Fernando', 'Airtable Mapping', 'Data mapping failed', level='warning')
        else:
            log_status('San Fernando', 'Scrape', 'No data scraped (no RFPs found or failed to parse table)', level='warning')
        # No failed_pages logic for San Fernando, but keep for consistency
        if san_fernando_stats and 'failed_pages' in san_fernando_stats:
            failed_pages = san_fernando_stats['failed_pages']
            if failed_pages:
                log_status('San Fernando', 'Failed URLs', f'{len(failed_pages)} failed URLs collected', level='warning')
                all_failed_urls.extend([{**page, 'source': 'San Fernando'} for page in failed_pages])
    except Exception as e:
        error_msg = f"San Fernando scraper failed to run: {e}"
        log_status('San Fernando', 'Error', error_msg, level='error')
        scraping_summary['san_fernando']['errors'].append(error_msg)
    ...
    try:
        ...
        if questcdn_stats and 'failed_pages' in questcdn_stats:
            failed_pages = questcdn_stats['failed_pages']
            if failed_pages:
                log_status('QuestCDN', 'Failed URLs', f'{len(failed_pages)} failed URLs collected', level='warning')
                all_failed_urls.extend([{**page, 'source': 'QuestCDN'} for page in failed_pages])
    except Exception as e:
    ...
    if all_failed_urls:
        print(f"\n📋 Saving failed URLs to failed_urls.txt...")
        print(f"   Total failed URLs across all portals: {len(all_failed_urls)}")
        
        # Group by source for better organization
        by_source = {}
        for failed_url in all_failed_urls:
            source = failed_url.get('source', 'Unknown')
            if source not in by_source:
                by_source[source] = []
            by_source[source].append(failed_url)
        
        # Save each source's failed URLs
        for source, urls in by_source.items():
            print(f"   {source}: {len(urls)} failed URLs")
            save_failed_pages_batch(urls, source)
